...
struct Graph{
    int numNodes;
    int degree;
    const double ** edgeleight;
    const double * states;
    double * temp;
};
inline int getNumNodes (struct Graph *graph){
    return graph->numNodes;
}
inline int getNeighbors (struct Graph *graph, int index){
    return graph->degree;
}
inline void gather2(int i,int j,struct Graph *graph, double *result){
    size_t count1 = i + m_ * j, count2 = j + m_ * i;
    double tmp = graph->states[i] * graph->states[j];
    result[0] += tmp * (graph->edgeleight[0][numbers*count1] + graph->edgeleight[0][numbers*count2]);
    result[1] += tmp * (graph->edgeleight[0][numbers*count1 + 1] + graph->edgeleight[0][numbers*count2 + 1]);
}
inline void apply2(int i,struct Graph *graph,double* result){
    size_t count = i + m_ * i;
    double tmp = graph->states[i] * graph->states[i];
    result[0] += tmp * graph->edgeleight[0][numbers*count];
    result[1] += tmp * graph->edgeleight[0][numbers*count + 1];
}
inline void GraphProcess2(struct Graph *graph, double * result){
    int nodes = getNumNodes(graph);
    int myid, numprocs;
    MPI_Comm_size(MPI_COMM_WORLD,&numprocs);
    MPI_Comm_rank(MPI_COMM_WORLD,&myid);
    omp_set_num_threads(threadNum);
    #pragma omp parallel for reduction(+:result[:2])
    for(int vi = myid;vi < nodes; vi+=numprocs){
        int neighbors = getNeighbors(graph,vi);
        for(int neighbor=0;neighbor < vi;neighbor++){
            gather2(vi,neighbor,graph,result);
        }
        apply2(vi,graph,result);
    }
}

inline void gather1(int i,int j,struct Graph *graph, double *result){
    size_t count1 = i + m_ * j, count2 = j + m_ * i;
    double tmp = graph->states[i] * graph->states[j];
    result[0] += tmp * (graph->edgeleight[0][count1] + graph->edgeleight[0][count2]);  
}
inline void apply1(int i,struct Graph *graph,double* result){
    size_t count = i + m_ * i;
    double tmp = graph->states[i] * graph->states[i];
    result[0] += tmp * graph->edgeleight[0][count];
    result[1] += graph->states[i] * graph->temp[i];
}
inline void GraphProcess1(struct Graph *graph, double * result){
    int nodes = getNumNodes(graph);
    int myid, numprocs;
    MPI_Comm_size(MPI_COMM_WORLD,&numprocs);
    MPI_Comm_rank(MPI_COMM_WORLD,&myid);
    ...
    for(int vi = myid;vi < nodes; vi+=numprocs){
        int neighbors = getNeighbors(graph,vi);
        for(int neighbor=0;neighbor < vi;neighbor++){
            gather1(vi,neighbor,graph,result);
        }
        apply1(vi,graph,result);
    }
}
...
void RedlichKwongMFTP::updateMixingExpressions()
{
    ...
    MPI_Comm_size(MPI_COMM_WORLD,&numprocs);
    MPI_Comm_rank(MPI_COMM_WORLD,&myid);
    double c[2];const double *a_[1] = {a_vec_Curr_.data()};const double *b_=moleFractions_.data();
    c[0] = 0;c[1] = 0;double *q = b_vec_Curr_.data();
    struct Graph graph={m_kk,m_kk,a_,b_,q};
    GraphProcess1(&graph,c);
    double x=0.0,y=0.0;
    MPI_Allreduce(&c[0], &x, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(&c[1], &y, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    m_a_current = x;
    m_b_current = y;
...
void RedlichKwongMFTP::calcCriticalConditions(doublereal& pc, doublereal& tc, doublereal& vc) const
{
    ...
    MPI_Comm_size(MPI_COMM_WORLD,&numprocs);
    MPI_Comm_rank(MPI_COMM_WORLD,&myid);
    double c[2];const double *a_[1] = {a_coeff_vec.data().data()};const double *b_=moleFractions_.data();
    c[0] = 0;c[1] = 0;double q[2] = {0};
    struct Graph graph={m_kk,m_kk,a_,b_,q};
    GraphProcess2(&graph,c);
    double x=0.0,y=0.0;
    MPI_Allreduce(&c[0], &x, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(&c[1], &y, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    a0 = x;
