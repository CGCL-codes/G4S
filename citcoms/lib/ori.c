...
// void gather(int e, int a, struct Graph* graph){
...
//   a = a + 1;
//   e = e + 1;
//   int ii1 = tempE->IEN[tempLevel][tempM][e].node[a];
//   int a11 = tempE->ID[tempLevel][tempM][ii1].doff[1];
//   int a12 = tempE->ID[tempLevel][tempM][ii1].doff[2];
//   int a13 = tempE->ID[tempLevel][tempM][ii1].doff[3];

//   double Au_a11 =  graph->temp[a11];
//   double Au_a12 =  graph->temp[a12];
//   double Au_a13 =  graph->temp[a13];

//   for(int b=1;b<=ends;b+=8) {
//     int nodeb1 = tempE->IEN[tempLevel][tempM][e].node[b];
//     int nodeb2 = tempE->IEN[tempLevel][tempM][e].node[b+1];
//     int nodeb3 = tempE->IEN[tempLevel][tempM][e].node[b+2];
//     int nodeb4 = tempE->IEN[tempLevel][tempM][e].node[b+3];
//     int nodeb5 = tempE->IEN[tempLevel][tempM][e].node[b+4];
//     int nodeb6 = tempE->IEN[tempLevel][tempM][e].node[b+5];
//     int nodeb7 = tempE->IEN[tempLevel][tempM][e].node[b+6];
//     int nodeb8 = tempE->IEN[tempLevel][tempM][e].node[b+7];

//     int ii1 = (a*n+b)*dims-(dims*n+dims);

//     double udoff11 = graph->states[tempE->ID[tempLevel][tempM][nodeb1].doff[1]];
//     double udoff12 = graph->states[tempE->ID[tempLevel][tempM][nodeb1].doff[2]];
//     double udoff13 = graph->states[tempE->ID[tempLevel][tempM][nodeb1].doff[3]];

//     double udoff21 = graph->states[tempE->ID[tempLevel][tempM][nodeb2].doff[1]];
//     double udoff22 = graph->states[tempE->ID[tempLevel][tempM][nodeb2].doff[2]];
//     double udoff23 = graph->states[tempE->ID[tempLevel][tempM][nodeb2].doff[3]];

//     double udoff31 = graph->states[tempE->ID[tempLevel][tempM][nodeb3].doff[1]];
//     double udoff32 = graph->states[tempE->ID[tempLevel][tempM][nodeb3].doff[2]];
//     double udoff33 = graph->states[tempE->ID[tempLevel][tempM][nodeb3].doff[3]];

//     double udoff41 = graph->states[tempE->ID[tempLevel][tempM][nodeb4].doff[1]];
//     double udoff42 = graph->states[tempE->ID[tempLevel][tempM][nodeb4].doff[2]];
//     double udoff43 = graph->states[tempE->ID[tempLevel][tempM][nodeb4].doff[3]];

//     double udoff51 = graph->states[tempE->ID[tempLevel][tempM][nodeb5].doff[1]];
//     double udoff52 = graph->states[tempE->ID[tempLevel][tempM][nodeb5].doff[2]];
//     double udoff53 = graph->states[tempE->ID[tempLevel][tempM][nodeb5].doff[3]];

//     double udoff61 = graph->states[tempE->ID[tempLevel][tempM][nodeb6].doff[1]];
//     double udoff62 = graph->states[tempE->ID[tempLevel][tempM][nodeb6].doff[2]];
//     double udoff63 = graph->states[tempE->ID[tempLevel][tempM][nodeb6].doff[3]];

//     double udoff71 = graph->states[tempE->ID[tempLevel][tempM][nodeb7].doff[1]];
//     double udoff72 = graph->states[tempE->ID[tempLevel][tempM][nodeb7].doff[2]];
//     double udoff73 = graph->states[tempE->ID[tempLevel][tempM][nodeb7].doff[3]];

//     double udoff81 = graph->states[tempE->ID[tempLevel][tempM][nodeb8].doff[1]];
//     double udoff82 = graph->states[tempE->ID[tempLevel][tempM][nodeb8].doff[2]];
//     double udoff83 = graph->states[tempE->ID[tempLevel][tempM][nodeb8].doff[3]];

//     Au_a11 +=
//               graph->edgeWeight[e][ii1]    * udoff11
//             + graph->edgeWeight[e][ii1+1]  * udoff12
//             + graph->edgeWeight[e][ii1+2]  * udoff13
//             + graph->edgeWeight[e][ii1+3]  * udoff21
//             + graph->edgeWeight[e][ii1+4]  * udoff22
//             + graph->edgeWeight[e][ii1+5]  * udoff23
//             + graph->edgeWeight[e][ii1+6]  * udoff31
//             + graph->edgeWeight[e][ii1+7]  * udoff32
//             + graph->edgeWeight[e][ii1+8]  * udoff33
//             + graph->edgeWeight[e][ii1+9]  * udoff41
//             + graph->edgeWeight[e][ii1+10] * udoff42
//             + graph->edgeWeight[e][ii1+11] * udoff43
//             + graph->edgeWeight[e][ii1+12] * udoff51
//             + graph->edgeWeight[e][ii1+13] * udoff52
//             + graph->edgeWeight[e][ii1+14] * udoff53
//             + graph->edgeWeight[e][ii1+15] * udoff61
//             + graph->edgeWeight[e][ii1+16] * udoff62
//             + graph->edgeWeight[e][ii1+17] * udoff63
//             + graph->edgeWeight[e][ii1+18] * udoff71
//             + graph->edgeWeight[e][ii1+19] * udoff72
//             + graph->edgeWeight[e][ii1+20] * udoff73
//             + graph->edgeWeight[e][ii1+21] * udoff81
//             + graph->edgeWeight[e][ii1+22] * udoff82
//             + graph->edgeWeight[e][ii1+23] * udoff83;
...
//             + graph->edgeWeight[e][ii1+23] * udoff83;
//   }
//   // printf("%f, %f, %f\n", Au_a11, Au_a12, Au_a13);
//   graph->temp[a11] = Au_a11;
//   graph->temp[a12] = Au_a12;
//   graph->temp[a13] = Au_a13;
// }
...
void gather(int e, int a, struct Graph* graph, double * u){
  ...
  int base = (a-1) * n * dims;
  for(int i = 1; i <=3; ++i){
    int begin = base + (i-1) * 24;
    int end = begin + 24;
    int a = tempE->ID[tempLevel][tempM][index].doff[i];
    for(int j = begin; j < end; ++j){
      graph->temp[a] += graph->edgeWeight[e][j] * u[j-begin];
    }
  }
}
...
//   // if(a == 0){
//   //   graph->temp = (double*)malloc(4 * ends * sizeof(double));
//   //   for(int i = 0; i < 4 * ends; ++i){
//   //     graph->temp[i] = 0.0;
//   //   }
//   // }
//   a = a+1;
...
//   int index = tempE->IEN[tempLevel][tempM][e].node[a];
//   int a1 = tempE->ID[tempLevel][tempM][index].doff[1];
//   int a2 = tempE->ID[tempLevel][tempM][index].doff[2];
//   int a3 = tempE->ID[tempLevel][tempM][index].doff[3];
//   // int extra = 0;
//   // for(int ele = 0; ele < 3; ++ele){
//   //   int dimn = tempE->ID[tempLevel][tempM][index].doff[ele+1];
//   //   int base = (a-1) * n * dims + extra;
//   //   int ii = base;
//   //   for(int b = 1; b <= ends; ++b){
//   //     int nodeb = tempE->IEN[tempLevel][tempM][e].node[b];
//   //     graph->temp[dimn] +=
// 	// 	        graph->edgeWeight[e][ii] * graph->states[tempE->ID[tempLevel][tempM][nodeb].doff[1]]
// 	// 	      + graph->edgeWeight[e][ii+1] * graph->states[tempE->ID[tempLevel][tempM][nodeb].doff[2]]
// 	// 	      + graph->edgeWeight[e][ii+2] * graph->states[tempE->ID[tempLevel][tempM][nodeb].doff[3]];
//   //     ii += 3;
//   //   }
//   //   extra += n;
//   // }

//   int base = (a*n)*dims-(dims*n+dims);
//   int ii = base;
//   int count = 0;
// 	for(int b=1;b<=ends;b++) {
// 	  int nodeb = tempE->IEN[tempLevel][tempM][e].node[b];
// 	  // int ii = (a*n+b)*dims-(dims*n+dims);
//     ii += 3;
// 		/* i=1, j=1,2 */
// 	    /* i=1, j=1,2,3 */
// 		graph->temp[a1] +=
...
// 		/* i=2, j=1,2,3 */
// 		graph->temp[a2] +=
// 		        graph->edgeWeight[e][ii+n] * graph->states[tempE->ID[tempLevel][tempM][nodeb].doff[1]]
// 		      + graph->edgeWeight[e][ii+n+1] * graph->states[tempE->ID[tempLevel][tempM][nodeb].doff[2]]
// 		      + graph->edgeWeight[e][ii+n+2] * graph->states[tempE->ID[tempLevel][tempM][nodeb].doff[3]];
// 		/* i=3, j=1,2,3 */
// 		graph->temp[a3] +=
// 		        graph->edgeWeight[e][ii+n+n] * graph->states[tempE->ID[tempLevel][tempM][nodeb].doff[1]]
// 		      + graph->edgeWeight[e][ii+n+n+1] * graph->states[tempE->ID[tempLevel][tempM][nodeb].doff[2]]
// 		      + graph->edgeWeight[e][ii+n+n+2] * graph->states[tempE->ID[tempLevel][tempM][nodeb].doff[3]];
//  	}
// }
...
void apply(int e, struct Graph *graph, double *Au){
  ...
  //   int temp = a*3;
  //   Au[a1] += graph->temp[temp];
  //   Au[a2] += graph->temp[temp+1];
  //   Au[a3] += graph->temp[temp+2];
  // }
  // free(graph->temp);
}

// typedef double* (*fun_t)(int, int, double **, double *, double *, va_list);
// typedef void (*fun_apply)(int, double *, double *, va_list);

// void spmv(int m, int n, double **a, double *b, double *c, fun_t gather, fun_apply apply, ...){
//   va_list arg_list;
//   for(int i=0;i<m;i++){
//     double *tmp = NULL;
//     for(int j=0;j<n;j++){
//       va_start(arg_list, apply);
//       tmp = gather(i, j, a, b, tmp, arg_list);
//     }
//     va_start(arg_list, apply);
//     apply(i, tmp, c, arg_list);
//   }
//   va_end(arg_list);
// }

void test(int nel, int ends, double **elt, double *u, double *Au, struct All_variables* E, int level, int m){
    const int dims=E->mesh.nsd;
    ...
    for(int e=1;e<=nel;e++){
      for(int a=1;a<=ends;a++){
        int ii = E->IEN[level][m][e].node[a];
        int a1 = E->ID[level][m][ii].doff[1];
        int a2 = E->ID[level][m][ii].doff[2];
        int a3 = E->ID[level][m][ii].doff[3];
        for(int b=1;b<=ends;b++) {
            int nodeb = E->IEN[level][m][e].node[b];
            ii = (a*n+b)*dims-(dims*n+dims);
        /* i=1, j=1,2 */
              /* i=1, j=1,2,3 */
      Au[a1] +=
              elt[e][ii] *
        u[E->ID[level][m][nodeb].doff[1]]
            + elt[e][ii+1] *
        u[E->ID[level][m][nodeb].doff[2]]
            + elt[e][ii+2] *
        u[E->ID[level][m][nodeb].doff[3]];
      /* i=2, j=1,2,3 */
      Au[a2] +=
              elt[e][ii+n] *
        u[E->ID[level][m][nodeb].doff[1]]
...

{
  ...
  int elt_k_index,ii_index,ai_index;
  tempLevel = level;
  tempE = E;
  for (m=1;m<=E->sphere.caps_per_proc;m++){
    for(i=0;i<neq;i++)
      ...
      tempM = m;
      // citcoms(nel,ends*ends,Au,E,level,m,u);
      test(nel, ends, E->elt_k[level][m], u[m], Au[m], E, level, m);
      // struct Graph graph;
      // graph.numNodes = nel;
      // graph.degree = ends;
      ...
      /*
      GraphProcess(&graph, Au[m], 
            [&](int e, int a, struct Graph* graph) {
                ...
            },
  ...
  //         elt_k_index = (a*n+b)*dims-(dims*n+dims);

  //         for(int ai=1;ai<=3;ai++){

  //           ai_index=E->ID[level][m][ii].doff[ai];

  //           ii_index = elt_k_index + n*(ai-1);

  //           // printf("Au[m][%d] E->elt_k[e][%d] u[%d]\n",ai_index,ii_index,E->ID[level][m][nodeb].doff[1]);
  //           double tmp=0.0;
  //           //gather
  //           for( int ui=1;ui<=3;ui++ ){
  //             tmp += E->elt_k[level][m][e].k[ii_index + ui - 1] * 
  //               u[m][E->ID[level][m][nodeb].doff[ui]];
  //           }
  //           //apply
  //           Au[m][ai_index] += tmp;
